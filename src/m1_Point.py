...
class Point(object):
    ...
    def move_to(self,x,y):
        self.travelled = self.travelled + math.sqrt((self.x-x)**2+(self.y-y)**2)
        self.x = x
        self.y = y
        self.moves = self.moves + 1

    def move_by(self,dx,dy):
        self.travelled =  self.travelled + math.sqrt((self.x-(self.x + dx))**2+(self.y-(self.y + dy))**2)
        self.x = self.x + dx
        self.y = self.y + dy
        self.moves = self.moves + 1
    ...
    def get_distance_from_start(self):
        return math.sqrt((self.x-self.xstart)**2+(self.y-self.ystart)**2)

    def get_distance_traveled(self):
        return self.travelled

    def closer_to(self,Point1,Point2):
        if math.sqrt((self.x-Point1.x)**2+(self.y-Point1.y)**2)<=math.sqrt((self.x-Point2.x)**2+(self.y-Point2.y)**2):
            return Point1
        else:
            return Point2

    def halfway_to(self,Point3):
        newPoint = Point((self.x+Point3.x)/2,(self.y+Point3.y)/2)
        return newPoint
